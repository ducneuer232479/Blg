...
export const addPost = createAsyncThunk('blog/addPost', async (body: Omit<Post, 'id'>, thunkAPI) => {
  try {
    ...
    if (error.name === 'AxiosError' && error.response.status === 425) {
      return thunkAPI.rejectWithValue(error.response.data)
    }
    // Báo cho createAsyncThunk 1 lỗi bình thường
    throw error
  }
})
  ...
  extraReducers(builder) {
    builder
      ...
      .addMatcher<PendingAction>(
        ...
        (state, action) => {
          state.loading = true
          // Khi chúng ta gọi AsyncThunk bất kỳ nào đấy, nó sẽ sinh ra 1 requestId unique, kể cả gọi 2 lần nên currentRequestId luôn khác nhau
